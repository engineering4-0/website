import { Button, Column, Grid, Stack, TextInput, FormGroup, Loading } from "@carbon/react";
...
const UserInfo = () => {
  ...
  const handleSubmit = async () => {
    if (email.length > 0 && instagramId.length > 0 && !emailError) {
      await axios.post(scriptUrl, { email: email, id: instagramId, createdAt: new Date() })
      .then(res => {
        console.log(res);
        setLoading(false);
        toast("Completed");
          ...
          <FormGroup style={{ maxWidth: '600px' }}
            ...
            <Stack gap={7}>
              <TextInput helperText=""
                ...
                name="email" required value={email} onChange={handleEmailChange} />
              <TextInput helperText="You can provide us your instagram handle to get a mention"
                id="instagram"
                invalidText="Invalid instagram handle."
                labelText="Instagram Handle"
                placeholder="@username"
                name="instagram" required value={instagramId}
                onChange={(e) => setInstagramId(e.target.value)} />
              <Button disabled={isLoading && emailError} onClick={handleSubmit} className="some-class" >
